...
class JointAndEELogger(Node):
    def __init__(self, logging_rate_hz=10):
        ...
        self.latest_joint_torques = None
        ...
        self.subscription_joint_torques = self.create_subscription(JointState, '/franka_robot_state_broadcaster/external_joint_torques', self.joint_torque_callback, 10)
    ...
    def joint_torque_callback(self, msg):
        """Store latest 7 joint torques."""
        if len(msg.effort) >= 7:
            self.latest_joint_torques = list(msg.effort[:7])
    ...
    def log_data(self):
        ...
        if self.latest_joint_positions is None or self.latest_ee_pose is None or self.latest_joint_torques is None:
            return
        current_time = time.time()
        if self.start_time is None:
            self.start_time = current_time
        elapsed_time = round(current_time - self.start_time, 6)
        row = [elapsed_time] + self.latest_joint_positions + self.latest_joint_torques + list(self.latest_ee_pose)
