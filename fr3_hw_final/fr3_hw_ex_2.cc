...
int main(int argc, char** argv) {
    ...
    const double ObstacleBounds3D[NO_OBSTACLES][6] = {
      ...
      { 0.3, 0.6, 0.05, 0.15, 0.2, 0.5 },
      { 0.3, 0.6, -0.15, -0.05, 0.2, 0.5 },

      //ONLY FOR T2
      { 0.3, 0.6, -0.3, 0.3, 0.6, 0.7 }
    ...
    auto target = [&ss, &s_eta](const abs_type& idx) {
        ...
        // if (!(0.35 <= (vz - hz) && (vz + hz) <= 0.4)) cond = false;
        ...
        double vy = x[1]; double hy = s_eta[1] / 2.0;
        if (!(0.15 <= (vy - hy) && (vy + hy) <= 0.25)) cond = false;
        ...
        // double vy = x[1]; double hy = s_eta[1] / 2.0;
        // if (!(-0.25 <= (vy - hy) && (vy + hy) <= -0.15)) cond = false;
    ...
    /* ================= CSV Export ================= */
    ofstream csvfile("fr3_hw_ex_2_t2.csv");
    // header
    csvfile << "x_m";
    csvfile << ",y_m";
    csvfile << ",z_m";
