...
int main(int argc, char** argv) {
    ...
    const size_t NO_OBSTACLES = 26;
    const double ObstacleBounds3D[NO_OBSTACLES][6] = {
        /* ================= STATE SPACE BOUNDARY ================= */
        {-0.28, 0.28, -0.28, 0.28, 0.02, 0.70},
        { 0.56, 0.70, -0.70, 0.70, 0.02, 0.70},
        {-0.70,-0.60, -0.70, 0.70, 0.02, 0.70},
        {-0.60, 0.56,  0.40, 0.70, 0.02, 0.70},
        {-0.60, 0.56, -0.70,-0.40, 0.02, 0.70},

        /* ================= INTERNAL BOUNDARY ================= */
        { 0.34, 0.56, -0.40, 0.40, 0.04, 0.70},
        { 0.15, 0.28, -0.40,-0.28, 0.02, 0.70},
        { 0.15, 0.28,  0.28, 0.40, 0.02, 0.70},
        { 0.28, 0.34,  0.02, 0.40, 0.04, 0.70},
        { 0.28, 0.34, -0.40,-0.02, 0.04, 0.70},

        /* ================= MAZE OBSTACLES ================= */
        { 0.28, 0.34, -0.06,-0.05, 0.02, 0.70},
        { 0.34, 0.35, -0.16, 0.32, 0.02, 0.04},
        { 0.35, 0.49,  0.31, 0.32, 0.02, 0.04},
        { 0.48, 0.56,  0.23, 0.24, 0.02, 0.04},
        { 0.35, 0.41,  0.23, 0.24, 0.02, 0.04},
        { 0.35, 0.49,  0.15, 0.16, 0.02, 0.04},
        { 0.42, 0.56,  0.07, 0.08, 0.02, 0.04},
        { 0.52, 0.56, -0.01, 0.00, 0.02, 0.04},
        { 0.35, 0.45, -0.01, 0.00, 0.02, 0.04},
        { 0.42, 0.56, -0.09,-0.08, 0.02, 0.04},
        { 0.32, 0.49, -0.17,-0.16, 0.02, 0.04},
        { 0.32, 0.56, -0.34,-0.33, 0.02, 0.04},
        { 0.48, 0.49, -0.33,-0.24, 0.02, 0.04},
        { 0.40, 0.41, -0.26,-0.17, 0.02, 0.04},
        { 0.32, 0.33, -0.33,-0.24, 0.02, 0.04}
    };
    ...
    write_to_file(ss, obstacle, "example_1_obstacle");
    ...
    auto target = [&ss, &s_eta](const abs_type& idx) {
        ...
        if (!(0.46 <= (vx - hx) && (vx + hx) <= 0.52)) cond = false;

        double vy = x[1]; double hy = s_eta[1] / 2.0;
        if (!(-0.4 <= (vy - hy) && (vy + hy) <= -0.34)) cond = false;
    ...
    write_to_file(ss, target, "example_1_target");
    ...
    cout << "\nWrite controller to example_1.scs \n";
    StaticController controller(ss, is, move(win));
    if (write_to_file(controller, "example_1")) {
        cout << "Done writing controller file. \n";
    }

    /* ================= CSV Export ================= */
    ofstream csvfile("example_1.csv");
    csvfile << "x";
    csvfile << ",y";
    csvfile << ",z";
    csvfile << ",vx";
    ...
    csvfile.close();
    cout << "State–input pairs written to example_1.csv (rows: " << rows_written << ")\n";
